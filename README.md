...
3. Clone or download this repository to your XAMPP **htdocs** folder.
   The final path should be `path_to/xampp/htdocs/mrms-balatan`.
...
9. Open your web browser and access <http://localhost:5198/mrms-balatan/> to view the application.
...
2. Inside [phpMyAdmin](http://localhost/phpmyadmin),
   create a MySQL database named `test-mrms-balatan` and import [missbuhi.sql](mrms-balatan.sql) into it.
...
2. Access the application by visiting `http://[host_name]/mrms-balatan`,
   where `host_name` is the **IP address** or **host name** of the server in the network.
   For example:
     - <http://localhost/mrms-balatan>
     - <http://192.168.1.99/mrms-balatan>


#### Admin Dashboard
Access the **admin dashboard** at <http://localhost/mrms-balatan/app>.
